...
fn main() -> Result<()> {
    ...
    let host = config.get("host").unwrap_or_else(|| panic!("Cannot found host in config"));
    let port=config.get("port").unwrap_or_else(|| panic!("Cannot found port in config"));
    let logfile = config.get("logfile").unwrap_or_else(|| panic!("Cannot found logfile in config"));
...
mod tests {
    ...
    fn test_get_contents(){
        ...
        assert_eq!(contents.unwrap(), "host: localhost\nport: 3333\nlogfile: ../log.txt")
    }
