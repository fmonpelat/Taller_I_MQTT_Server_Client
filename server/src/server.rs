...
use std::thread;
use std::thread::JoinHandle; //, channel};
...
impl Server {
                        ...
                        if _size > 0 {
                            ...
                            if Packet::<VariableHeader, Payload>::is_mqtt_packet(&buff) {
                                ...
                                match Server::handle_packet(
                                    ...
                                    Ok(_) => {
                                        logger.debug(format!(
                                            "Peer {} succefully connect with server",
                                            stream.peer_addr()?
                                        ));
    ...
    fn handle_packet(
        ...
        if (packet_id == control_type::CONNECT)
            && Server::get_id_persistance_connections(peer_addr, hash_persistance_connections)?
        {
    ...
    fn recalculate_buff(buff: &mut [u8; 1024], copy_buff: &mut [u8; 1024]) -> [u8; 1024] {
        ...
        let remaining_len = Packet::<VariableHeader, Payload>::get_packet_length(
            &buff[1..buff.len()].to_vec(),
            &mut buff_readed,
        );
        copy_buff[(remaining_len + buff_readed + 1)..buff.len()]
            .clone_from_slice(&buff[(remaining_len + buff_readed + 1)..buff.len()]);
        *copy_buff
    }
    ...
    fn _get_hash_server_connections(
        peer_addr: SocketAddr,
        hash_server_connections: Arc<Mutex<HashServerConnections>>,
    ) -> Result<bool> {
